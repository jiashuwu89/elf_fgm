...
def fgm_calib(starttime_str: str, endtime_str: str, sta_cdfpath: str, fgm_cdfpath: str):
    ...
    pos_cdfdata["ela_pos_time"] = pd.date_range(
        start="2022-01-12", periods=len(pos_cdfdata["ela_pos_gei"]), freq="S"
    )
    ...
    for i in range(1, len(ctime) - 2):
        if (
            d_B_S3_corr[i - 1] > 0
            and d_B_S3_corr[i] > 0
            and d_B_S3_corr[i + 1] < 0
            and d_B_S3_corr[i + 2] < 0
            and B_S3_corr[i - 1] > 0
            and B_S3_corr[i] > 0
            and B_S3_corr[i + 1] > 0
            and B_S3_corr[i + 2] > 0
    ...
    cross_times_d_B_S3_corr_1_mids = 0.5 * (
        cross_times_d_B_S3_corr_1[1:] + cross_times_d_B_S3_corr_1[:-1]
    )
    ...
    valid_idx_corr_1 = running_filter(
        cross_times_d_B_S3_corr_1_mids, w_syn_d_corr_1, eps_corr_1
    )

    # Select the corresponding crossing time mid-points, synodic angular velocities and spin-periods
    cross_times_d_B_S3_corr_1_mids_select = cross_times_d_B_S3_corr_1_mids[
        valid_idx_corr_1
    ]
    ...
    cross_times_d_B_S3_corr_1_select = (
        cross_times_d_B_S3_corr_1_mids_select - T_spins_d_corr_1_select / 2
    )
    cross_times_d_B_S3_corr_1_select = np.concatenate(
        (
            cross_times_d_B_S3_corr_1_select,
            np.array(
                [
                    cross_times_d_B_S3_corr_1_mids_select[-1]
                    + T_spins_d_corr_1_select[-1] / 2
                ]
            ),
        )
    )
    ...
    if proper_pad == True:
        if fit_running_spline == True:
            T_spins_d_pad_corr_1_select = running_spline(
                cross_times_d_B_S3_corr_1_select,
                cross_times_d_B_S3_corr_1_mids_select,
                T_spins_d_corr_1_select,
                T=30,
            )
        else:
            T_spins_d_pad_corr_1_select = func(
                cross_times_d_B_S3_corr_1_select,
                *curve_fit(
                    func, cross_times_d_B_S3_corr_1_mids_select, T_spins_d_corr_1_select
                )[0],
            )
    else:
    ...
    for i in range(len(cross_times_d_B_S3_corr_1_select)):
        ...
        idx = ((ctime - t0) >= -T_spins_d_pad_corr_1_select[i] / 2) & (
            (ctime - t0) <= T_spins_d_pad_corr_1_select[i] / 2
        )
    ...
    for i in range(1, len(ctime) - 2):
        if (
            phase_corr[i - 1] > 0
            and phase_corr[i] > 0
            and phase_corr[i + 1] < 0
            and phase_corr[i + 2] < 0
            and B_S3_corr[i - 1] > 0
            and B_S3_corr[i] > 0
            and B_S3_corr[i + 1] > 0
            and B_S3_corr[i + 2] > 0
    ...
    cross_times_d_B_S3_corr_2_mids = 0.5 * (
        cross_times_d_B_S3_corr_2[1:] + cross_times_d_B_S3_corr_2[:-1]
    )
    ...
    ############################################
    eps_corr_2 = 1e5
    valid_idx_corr_2 = running_filter(
        cross_times_d_B_S3_corr_2_mids, w_syn_d_corr_2, eps_corr_2
    )

    cross_times_d_B_S3_corr_2_mids_select = cross_times_d_B_S3_corr_2_mids[
        valid_idx_corr_2
    ]
    ...
    cross_times_d_B_S3_corr_2_select = (
        cross_times_d_B_S3_corr_2_mids_select - T_spins_d_corr_2_select / 2
    )
    cross_times_d_B_S3_corr_2_select = np.concatenate(
        (
            cross_times_d_B_S3_corr_2_select,
            np.array(
                [
                    cross_times_d_B_S3_corr_2_mids_select[-1]
                    + T_spins_d_corr_2_select[-1] / 2
                ]
            ),
        )
    )
    ...
    if proper_pad == True:
        if fit_running_spline == True:
            T_spins_d_pad_corr_2_select = running_spline(
                cross_times_d_B_S3_corr_2_select,
                cross_times_d_B_S3_corr_2_mids_select,
                T_spins_d_corr_2_select,
                T=30,
            )
        else:
            T_spins_d_pad_corr_2_select = func(
                cross_times_d_B_S3_corr_2_select,
                *curve_fit(
                    func, cross_times_d_B_S3_corr_2_mids_select, T_spins_d_corr_2_select
                )[0],
            )
    else:
    ...
    ############################################
    eps_corr_3 = 2
    valid_idx_corr_3 = running_filter(
        cross_times_d_B_S3_corr_3, w_syn_d_corr_3, eps_corr_3, T=50
    )
    ...
    if fit_running_spline == True:
        if zero_crossing_method == 1 or zero_crossing_method == 2:
            w_syn_corr = running_spline(
                ctime, cross_times_d_B_S3_corr_mids, w_syn_d_corr, T=30
            )
        else:
            w_syn_corr = running_spline(
                ctime, cross_times_d_B_S3_corr, w_syn_d_corr, T=50
            )

    else:
        if zero_crossing_method == 1 or zero_crossing_method == 2:
            w_syn_corr = func(
                ctime, *curve_fit(func, cross_times_d_B_S3_corr_mids, w_syn_d_corr)[0]
            )
        else:
            w_syn_corr = func(
                ctime, *curve_fit(func, cross_times_d_B_S3_corr, w_syn_d_corr)[0]
            )

    if relative_integrate == True:
        # Use multiple reference points for integration
        idx0_corrs = np.array(
            [np.where(ctime <= t0_corr)[0][-1] for t0_corr in cross_times_d_B_S3_corr]
        )
        if fit_running_spline == True:
            if zero_crossing_method == 1 or zero_crossing_method == 2:
                w_t0_corrs = running_spline(
                    cross_times_d_B_S3_corr,
                    cross_times_d_B_S3_corr_mids,
                    w_syn_d_corr,
                    T=30,
                )
            else:
                w_t0_corrs = running_spline(
                    cross_times_d_B_S3_corr, cross_times_d_B_S3_corr, w_syn_d_corr, T=50
                )
        else:
            if zero_crossing_method == 1 or zero_crossing_method == 2:
                w_t0_corrs = func(
                    cross_times_d_B_S3_corr,
                    *curve_fit(func, cross_times_d_B_S3_corr_mids, w_syn_d_corr)[0],
                )
            else:
                w_t0_corrs = func(
                    cross_times_d_B_S3_corr,
                    *curve_fit(func, cross_times_d_B_S3_corr, w_syn_d_corr)[0],
                )
    else:
        ...
        if fit_running_spline == True:
            if zero_crossing_method == 1 or zero_crossing_method == 2:
                w_t0_corr = running_spline(
                    [t0_corr], cross_times_d_B_S3_corr_mids, w_syn_d_corr, T=30
                )[0]
            else:
                w_t0_corr = running_spline(
                    [t0_corr], cross_times_d_B_S3_corr, w_syn_d_corr, T=50
                )[0]
        else:
            if zero_crossing_method == 1 or zero_crossing_method == 2:
                w_t0_corr = func(
                ...
                w_t0_corr = func(
                    t0_corr, *curve_fit(func, cross_times_d_B_S3_corr, w_syn_d_corr)[0]
                )
    ...
    for i in range(len(phi_corr)):
            ...
            if bidirectional_integrate == True:
                ...
                if np.sum((ctime[i] - cross_times_d_B_S3_corr) > 0) == 0:
                    ...
                    t0_corr_idx = np.nanargmin(
                        mask_neg(ctime[i] - cross_times_d_B_S3_corr)
                    )
        ...
        if i < idx0_corr:
            phi_corr[i] = -simpson(
                w_syn_corr[i : idx0_corr + 1], x=ctime[i : idx0_corr + 1], dx=delta_t
            )
        elif i > idx0_corr:
            phi_corr[i] = simpson(
                w_syn_corr[idx0_corr : i + 1], x=ctime[idx0_corr : i + 1], dx=delta_t
            )
        else:
            phi_corr[i] = 0
        ...
        phi_corr[i] -= (
            0.5 * (w_t0_corr + w_syn_corr[idx0_corr]) * (t0_corr - ctime[idx0_corr])
        )
    ...
    B_hat_IGRF_GEI_x = B_IGRF_GEI_x / np.sqrt(
        B_IGRF_GEI_x**2 + B_IGRF_GEI_y**2 + B_IGRF_GEI_z**2
    )
    B_hat_IGRF_GEI_y = B_IGRF_GEI_y / np.sqrt(
        B_IGRF_GEI_x**2 + B_IGRF_GEI_y**2 + B_IGRF_GEI_z**2
    )
    B_hat_IGRF_GEI_z = B_IGRF_GEI_z / np.sqrt(
        B_IGRF_GEI_x**2 + B_IGRF_GEI_y**2 + B_IGRF_GEI_z**2
    )
    ...
    for i in range(len(ctime)):
        ...
        b_hat = np.array(
            [B_hat_IGRF_GEI_x[i], B_hat_IGRF_GEI_y[i], B_hat_IGRF_GEI_z[i]]
        )
    ...
    for i in range(len(ctime)):
        ...
        B_x_DMXL_true[i] = (
            GEI_2_DMXL[0, 0] * B_IGRF_GEI_x[i]
            + GEI_2_DMXL[0, 1] * B_IGRF_GEI_y[i]
            + GEI_2_DMXL[0, 2] * B_IGRF_GEI_z[i]
        )
        B_y_DMXL_true[i] = (
            GEI_2_DMXL[1, 0] * B_IGRF_GEI_x[i]
            + GEI_2_DMXL[1, 1] * B_IGRF_GEI_y[i]
            + GEI_2_DMXL[1, 2] * B_IGRF_GEI_z[i]
        )
        B_z_DMXL_true[i] = (
            GEI_2_DMXL[2, 0] * B_IGRF_GEI_x[i]
            + GEI_2_DMXL[2, 1] * B_IGRF_GEI_y[i]
            + GEI_2_DMXL[2, 2] * B_IGRF_GEI_z[i]
        )
    B_x_SMXL_corr_respin = (
        np.cos(-phi_corr) * B_x_DMXL_true - np.sin(-phi_corr) * B_y_DMXL_true
    )
    B_y_SMXL_corr_respin = (
        np.sin(-phi_corr) * B_x_DMXL_true + np.cos(-phi_corr) * B_y_DMXL_true
    )
    ...
    cross_times_d_B_S3_calib_1_mids = 0.5 * (
        cross_times_d_B_S3_calib_1[1:] + cross_times_d_B_S3_calib_1[:-1]
    )
    ...
    valid_idx_calib_1 = running_filter(
        cross_times_d_B_S3_calib_1_mids, w_syn_d_calib_1, eps_calib_1
    )
    cross_times_d_B_S3_calib_1_mids_select = cross_times_d_B_S3_calib_1_mids[
        valid_idx_calib_1
    ]
    ...
    cross_times_d_B_S3_calib_1_select = (
        cross_times_d_B_S3_calib_1_mids_select - T_spins_d_calib_1_select / 2
    )
    cross_times_d_B_S3_calib_1_select = np.concatenate(
        (
            cross_times_d_B_S3_calib_1_select,
            np.array(
                [
                    cross_times_d_B_S3_calib_1_mids_select[-1]
                    + T_spins_d_calib_1_select[-1] / 2
                ]
            ),
        )
    )
    ...
    if proper_pad == True:
        if fit_running_spline == True:
            T_spins_d_pad_calib_1_select = running_spline(
                cross_times_d_B_S3_calib_1_select,
                cross_times_d_B_S3_calib_1_mids_select,
                T_spins_d_calib_1_select,
                T=30,
            )
        else:
            T_spins_d_pad_calib_1_select = func(
                cross_times_d_B_S3_calib_1_select,
                *curve_fit(
                    func,
                    cross_times_d_B_S3_calib_1_mids_select,
        ...
        T_spins_d_pad_calib_1_select = np.append(
            T_spins_d_calib_1, T_spins_d_calib_1[-1]
        )
    ...
    for i in range(len(cross_times_d_B_S3_calib_1_select)):
        ...
        idx = ((ctime - t0) >= -T_spins_d_pad_calib_1_select[i] / 2) & (
            (ctime - t0) <= T_spins_d_pad_calib_1_select[i] / 2
        )
        phase_calib[idx] = np.arcsin(
            d_B_S3_calib[idx] / np.max(np.abs(d_B_S3_calib[idx]))
        )
    ...
    cross_times_d_B_S3_calib_2_mids = 0.5 * (
        cross_times_d_B_S3_calib_2[1:] + cross_times_d_B_S3_calib_2[:-1]
    )
    ...
    valid_idx_calib_2 = running_filter(
        cross_times_d_B_S3_calib_2_mids, w_syn_d_calib_2, eps_calib_2
    )

    cross_times_d_B_S3_calib_2_mids_select = cross_times_d_B_S3_calib_2_mids[
        valid_idx_calib_2
    ]
    ...
    cross_times_d_B_S3_calib_2_select = (
        cross_times_d_B_S3_calib_2_mids_select - T_spins_d_calib_2_select / 2
    )
    cross_times_d_B_S3_calib_2_select = np.concatenate(
        (
            cross_times_d_B_S3_calib_2_select,
            np.array(
                [
                    cross_times_d_B_S3_calib_2_mids_select[-1]
                    + T_spins_d_calib_2_select[-1] / 2
                ]
            ),
        )
    )
    ...
    if proper_pad == True:
        if fit_running_spline == True:
            T_spins_d_pad_calib_2_select = running_spline(
                cross_times_d_B_S3_calib_2_select,
                cross_times_d_B_S3_calib_2_mids_select,
                T_spins_d_calib_2_select,
                T=30,
            )
        else:

            T_spins_d_pad_calib_2_select = func(
                cross_times_d_B_S3_calib_2_select,
                *curve_fit(
                    func,
                    cross_times_d_B_S3_calib_2_mids_select,
        ...
        T_spins_d_pad_calib_2_select = np.append(
            T_spins_d_calib_2, T_spins_d_calib_2[-1]
        )
    ...
    for i in range(len(cross_times_d_B_S3_calib_2_select)):
        ...
        R2score.append(
            1
            - sum((signal_slice - y_model) ** 2)
            / sum((signal_slice - np.mean(signal_slice)) ** 2)
        )
    ...
    valid_idx_calib_3 = running_filter(
        cross_times_d_B_S3_calib_3, w_syn_d_calib_3, eps_calib_3, T=50
    )
    ...
    if fit_running_spline == True:
        if zero_crossing_method == 1 or zero_crossing_method == 2:
            w_syn_calib = running_spline(
                ctime, cross_times_d_B_S3_calib_mids, w_syn_d_calib, T=25
            )
        else:
            w_syn_calib = running_spline(
                ctime, cross_times_d_B_S3_calib, w_syn_d_calib, T=50
            )
    else:
        if zero_crossing_method == 1 or zero_crossing_method == 2:
            w_syn_calib = func(
                ctime, *curve_fit(func, cross_times_d_B_S3_calib_mids, w_syn_d_calib)[0]
            )
        else:
            w_syn_calib = func(
                ctime, *curve_fit(func, cross_times_d_B_S3_calib, w_syn_d_calib)[0]
            )
    ...
    ############################################
    if relative_integrate == True:
        idx0_calibs = np.array(
            [
                np.where(ctime <= t0_calib)[0][-1]
                for t0_calib in cross_times_d_B_S3_calib
            ]
        )

        if fit_running_spline == True:
            if zero_crossing_method == 1 or zero_crossing_method == 2:
                w_t0_calibs = running_spline(
                    cross_times_d_B_S3_calib,
                    cross_times_d_B_S3_calib_mids,
                    w_syn_d_calib,
                    T=30,
                )
            else:
            ...
            if zero_crossing_method == 1 or zero_crossing_method == 2:
                w_t0_calibs = func(
                    cross_times_d_B_S3_calib,
                    *curve_fit(func, cross_times_d_B_S3_calib_mids, w_syn_d_calib)[0],
                )
            else:
                w_t0_calibs = func(
                    cross_times_d_B_S3_calib,
                    *curve_fit(func, cross_times_d_B_S3_calib, w_syn_d_calib)[0],
                )
    else:
        ...
        if fit_running_spline == True:
            if zero_crossing_method == 1 or zero_crossing_method == 2:
                w_t0_calib = running_spline(
                    [t0_calib], cross_times_d_B_S3_calib_mids, w_syn_d_calib, T=30
                )[0]
            else:
                w_t0_calib = running_spline(
                    [t0_calib], cross_times_d_B_S3_calib, w_syn_d_calib, T=50
                )[0]
        else:
            if zero_crossing_method == 1 or zero_crossing_method == 2:
                w_t0_calib = func(
    ...
    for i in range(len(phi_calib)):
        if relative_integrate == True:
            if bidirectional_integrate == True:
                ...
                if np.sum((ctime[i] - cross_times_d_B_S3_calib) > 0) == 0:
                    ...
                    t0_calib_idx = np.nanargmin(
                        mask_neg(ctime[i] - cross_times_d_B_S3_calib)
                    )
        ...
        if i < idx0_calib:
            phi_calib[i] = -simpson(
                w_syn_calib[i : idx0_calib + 1], x=ctime[i : idx0_calib + 1], dx=delta_t
            )
        elif i > idx0_calib:
            phi_calib[i] = simpson(
                w_syn_calib[idx0_calib : i + 1], x=ctime[idx0_calib : i + 1], dx=delta_t
            )
        else:
            phi_calib[i] = 0
        ...
        phi_calib[i] -= (
            0.5
            * (w_t0_calib + w_syn_calib[idx0_calib])
            * (t0_calib - ctime[idx0_calib])
        )
    ...
    B_x_DMXL_calib = (
        np.cos(phi_calib) * B_x_SMXL_calib - np.sin(phi_calib) * B_y_SMXL_calib
    )
    B_y_DMXL_calib = (
        np.sin(phi_calib) * B_x_SMXL_calib + np.cos(phi_calib) * B_y_SMXL_calib
    )
    ...
    for i in range(len(ctime)):
        IGRF_x_gei[i] = (
            DMXL_2_GEI[i][0, 0] * B_x_DMXL_true[i]
            + DMXL_2_GEI[i][0, 1] * B_y_DMXL_true[i]
            + DMXL_2_GEI[i][0, 2] * B_z_DMXL_true[i]
        )
        IGRF_y_gei[i] = (
            DMXL_2_GEI[i][1, 0] * B_x_DMXL_true[i]
            + DMXL_2_GEI[i][1, 1] * B_y_DMXL_true[i]
            + DMXL_2_GEI[i][1, 2] * B_z_DMXL_true[i]
        )
        IGRF_z_gei[i] = (
            DMXL_2_GEI[i][2, 0] * B_x_DMXL_true[i]
            + DMXL_2_GEI[i][2, 1] * B_y_DMXL_true[i]
            + DMXL_2_GEI[i][2, 2] * B_z_DMXL_true[i]
        )
    ...
    for i in range(len(ctime)):
        B_x_gei[i] = (
            DMXL_2_GEI[i][0, 0] * B_x_DMXL_calib[i]
            + DMXL_2_GEI[i][0, 1] * B_y_DMXL_calib[i]
            + DMXL_2_GEI[i][0, 2] * B_z_DMXL_calib[i]
        )
        B_y_gei[i] = (
            DMXL_2_GEI[i][1, 0] * B_x_DMXL_calib[i]
            + DMXL_2_GEI[i][1, 1] * B_y_DMXL_calib[i]
            + DMXL_2_GEI[i][1, 2] * B_z_DMXL_calib[i]
        )
        B_z_gei[i] = (
            DMXL_2_GEI[i][2, 0] * B_x_DMXL_calib[i]
            + DMXL_2_GEI[i][2, 1] * B_y_DMXL_calib[i]
            + DMXL_2_GEI[i][2, 2] * B_z_DMXL_calib[i]
        )
...
if __name__ == "__main__":
    ...
    [
        ...
        IGRF_smooth_y_gei,
        IGRF_smooth_z_gei,
    ] = fgm_calib(starttime_str, endtime_str, sta_cdfpath, fgm_cdfpath)
